...
const TaskDetail = () => {
  ...
  const isShowExtendTaskInfo = useMemo(() => {
    ...
    if ([IStatus.CODEING,IStatus.WAIT_ACCEPT, IStatus.COMPLETE].includes(data?.taskStatus.toString())) {
      return true
    }
    return false
  }, [data?.taskStatus])
      ...
      <Portal>
        ...
        <Box display="flex" gap="20px">
          <Flex direction="column">
            ...
            {isShowExtendTaskInfo && <TaskPlanList from={IPath.MYTASKS} completePlan={completePlan} data={data} address={userInfo.address} planlist={planlist}/>}
            {/* 待签约-我的投标； 其他： 投标记录 */}
            <TaskBidRecords from={IPath.MYTASKS} recordList={data?.assetRecordList} taskStatus={data?.taskStatus} openRecordDetail={handleOpenRecordDetail}/>
            <TaskDescription description={data?.projectRawInfo?.description} fileList={data?.fileList}/>
          </Flex>
          <Flex direction="column">
            <TaskStatusInfo from={IPath.MYTASKS} taskStatus={data.taskStatus?.toString()} scheduleTask={() => setSchedule(true)} submitAccept={submitAccept} withdrawMyRewards={withdrawMyRewards}/>
            {/* TODO 待获取 */}
