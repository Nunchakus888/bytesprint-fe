...
import { Box, Button, Flex, FormControl, FormErrorMessage, Input, InputGroup, InputRightElement, Link, styled, Text } from "@chakra-ui/react";
      ...
      render: (_:any, record:any, index: number) => {
        return (
          ...
          <FormControl isInvalid={!!errors?.datas?.[index]?.workhours} isRequired>
            <Input key={`datas.${index}.workhours`} color="#fff" type="number" placeholder='请输入' size='md' {...register(`datas.${index}.workhours`, { required: true })}  /> 
            <FormErrorMessage>
              {errors?.datas?.[index]?.workhours && <>{errors?.datas?.[index]?.workhours.message}</>}
            </FormErrorMessage>
        </FormControl>
        )
      },
      ...
      render: (_:any, record:any, index: number) => {
        return (
          <FormControl isInvalid={!!errors?.datas?.[index]?.startTime} isRequired>
            ...
            <FormErrorMessage>
              {errors?.datas?.[index]?.startTime && <>{errors?.datas?.[index]?.startTime.message}</>}
            </FormErrorMessage>
        </FormControl>
        )
      },
          ...
          fields.map((it,index) => {
            return (
              <Flex key={`line_${index}`} justify="space-between" padding="10px 0">
                ...
                <Flex alignItems="center" justifyContent="center" width="300px">{it.taskname}</Flex>
                <Flex alignItems="center" justifyContent="center" width="300px" paddingRight="10px">
                  <FormControl isInvalid={!!errors?.datas?.[index]?.workhours} isRequired>
                    <Input key={`datas.${index}.workhours`} color="#fff" type="number" placeholder='请输入' size='md' {...register(`datas.${index}.workhours`, { required: true, min:0 })}  /> 
                    <FormErrorMessage>
                      {errors?.datas?.[index]?.workhours && <>{errors?.datas?.[index]?.workhours.message}</>}
                    </FormErrorMessage>
                  </FormControl>
